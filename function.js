...
const runwayThemes = [
  ...
  "Before and After"
    ...
    function generateQueenInputs() {
      const count = parseInt(document.getElementById('queens-count').value);
      ...
      for (let i = 1; i <= count; i++) {
        ...
        for (let j = 1; j <= count; j++) {
          if (i !== j) {
            const input = document.getElementById(`relationship-${i}-${j}`);
            if (input) {
              gameState.relationships[i][j] = parseInt(input.value);
            }
          }
        }
      }
    ...
    function showBottomQueens() {
      ...
      bottomQueensContainer.innerHTML = `
        ...
        <div class="queen-list">
          ${gameState.bottomQueens.map(queen => `
            <div class="queen-card bottom">
              <img src="${queen.imageUrl}" alt="${queen.name}" class="queen-image">
              <h3>${queen.name}</h3>
            </div>
    ...
    function selectSecondQueen() {
      ...
      const remainingBottomQueens = gameState.bottomQueens.filter(
        queen => queen.id !== gameState.lipsynQueen1.id
      );
      
      const remainingContainer = document.getElementById('remaining-bottom-queens');
      remainingContainer.innerHTML = `
        <div class="queen-list">
          ${remainingBottomQueens.map(queen => `
      ...
      setTimeout(() => {
        ...
        const otherBottomQueens = remainingBottomQueens.filter(
          queen => queen.id !== gameState.lipsynQueen2.id
        );
    ...
    function determineLipsynWinner() {
      ...
      const queen1Description = lipsynPerformances[queen1Level][Math.floor(Math.random() * lipsynPerformances[queen1Level].length)];
      const queen2Description = lipsynPerformances[queen2Level][Math.floor(Math.random() * lipsynPerformances[queen2Level].length)];
  ...
  const sortedQueens = [...gameState.queens].sort((a, b) => {
    ...
    if (a.eliminated && b.eliminated) {
      ...
      const aPlace = a.placement ? parseInt(a.placement) : 999;
      const bPlace = b.placement ? parseInt(b.placement) : 999;
  ...
  sortedQueens.forEach(queen => {
    ...
    const totalEpisodes = queen.wins + queen.highs + queen.safes + queen.lows + queen.bottoms + (queen.eliminated ? 1 : 0);
  ...
  showScreen(trackRecordScreen);
    ...
    function determineFinaleWinner() {
      ...
      const pair = gameState.finaleRound === 0 
      ...
      setTimeout(() => {
        ...
        if (gameState.finaleRound === 0) {
          ...
          const semifinalists = gameState.finaleQueens.filter(q => 
            q.id !== gameState.finalePairs[0][0].id && 
            q.id !== gameState.finalePairs[1][0].id
          );
